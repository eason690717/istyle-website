...
import {
  scrapeSource1, scrapeSource2, scrapeSource3, scrapeUs3cAirPods, scrapeUs3cAndroid,
...
async function main() {
  const t0 = Date.now();
  const [s1, s2, s3, sA, sD] = await Promise.all([
    scrapeSource1(), scrapeSource2(), scrapeSource3(), scrapeUs3cAirPods(), scrapeUs3cAndroid(),
  ]);
  ...
  report("source2-android (us3c)", sD);
  ...
  const k1 = nk(s1), k2 = nk([...s2, ...sA, ...sD]), k3 = nk(s3);
  ...
  console.log(`  只有 s3 有的: ${only3.length} 筆（多為只有 jyes 提供的非 Apple 機型）`);
